/**
 * Swing Signals Pro — multi-timeframe trend, card per index, last 4 buy signals.
...
export default function SwingSignalsPro() {
  ...
  const loadHistory = useCallback(() => {
    proApi.buySignalHistory().then((list) => {
      if (Array.isArray(list) && list.length > 0) {
        const seen = new Set();
        const isSwing = (e) => (e.reason || "").toLowerCase().includes("swing");
        setHistory(
          list
            .filter((e) => (e.signal === "Buy CE" || e.signal === "Buy PE") && isSwing(e))
            .filter((e) => {
  ...
  const captureSignal = useCallback(async (symbol, signal, level) => {
    ...
    const key = `swing|${symbol}|${signal}`;
    if (lastSavedRef.current[key] && Date.now() - lastSavedRef.current[key] < SAVE_COOLDOWN_MS) return;
    lastSavedRef.current[key] = Date.now();
    const saved = await proApi.saveBuySignal({ symbol, signal, level, momentum: null, reason: "Swing signal" });
    if (saved) {
      setHistory((h) => {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <span className="text-sm font-bold text-slate-100">📈 Swing Signals</span>
        <span className="text-[10px] text-slate-500">Multi-timeframe trend · Updates every 15s</span>
      </div>
